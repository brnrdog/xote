...
Two sharp edges worth knowing: `MaybeSignal.map` runs its function once immediately and, for reactive values, leaves a `Computed` subscribed to the source — created during a component's render pass it is registered with the active owner and disposed on unmount, anywhere else (including a *re-render* of a reactive region, an effect body, or an event handler, where no owner is active) it must be `Computed.dispose`d by hand, and reading one after its owner was disposed silently returns the last computed value; and `MaybeSignal.toSignal` returns `Some(source signal)` for `Reactive` but `None` for `Static`, because a plain value has no underlying signal — lift one explicitly with `MaybeSignal.fold(value, ~static=v => Signal.make(v), ~reactive=s => s)` when you need a uniform `Signal.t`.
