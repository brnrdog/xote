...
- **`Xote.MaybeSignal`**: Static-or-reactive value wrapper exposing the type `t<'a> = Reactive(Signal.t<'a>) | Static('a)` plus `static`, `reactive`, `computed` (derives a `Reactive` from a `unit => 'a`), `fold` (the single total branch over the two cases), `get` (tracked read), `peek` (untracked read), `isStatic`/`isReactive`, `map`, `toSignal`, and `ofUnknown`. Use it anywhere an API should accept either a plain value or a signal — JSX props are the most common case, not the only one. Notes on the trickier members:
  ...
  - `map` runs its function once immediately in both cases. For a `Reactive` value the result is backed by a `Computed`; when it is created **during a component's render pass** that `Computed` is registered with the active owner and disposed on unmount, and anywhere else you must call `Computed.dispose` on it yourself. Two consequences worth knowing:
    - The owner is only active on the render pass itself. `runWithOwner` restores the previous owner when it returns, so a `map` (or `computed`) called from a *re-render* of a reactive region, an effect body, or an event handler runs with no owner and is not tracked.
    - Owner disposal is silent: reading a mapped value after its owner was disposed returns the last computed value forever, with no error. If a derived value must outlive the component that created it, build it with `MaybeSignal.reactive(Computed.make(fn))` — that spelling skips owner registration — and dispose it yourself.
...
4. **Computed disposal**: `Computed.make` returns a `Signal.t<'a>` directly, already subscribed to whatever it read during its initial computation. There is no automatic disposal — call `Computed.dispose(signal)` to unlink a computed you no longer need. Xote's owner system tracks `Effect` disposers and, for computeds created via `MaybeSignal.computed`/`MaybeSignal.map`/`View.render`, the resulting `Computed` as well; a raw `Computed.make` during render is still your responsibility to dispose. That tracking covers the render pass only — a computed created while a reactive region *re-renders* has no active owner and is not disposed for you.
