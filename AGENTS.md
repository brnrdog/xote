...
**Xote Modules:**
- **`Xote.View`**: Core rendering primitives. Defines the virtual node types (`Element`, `Text`, `SignalText`, `Fragment`, `SignalFragment`, `Keyed`, `LazyComponent`, `KeyedList`) and exposes node constructors (`text`, `signalText`, `signalInt`, `signalFloat`, `int`, `float`, `bool`, `fragment`, `signalFragment`, `tracked`, `each`, `eachWithKey`, `element`, and `render` — a `MaybeSignal.t`-to-`node` combinator that renders a `Static` value once and a `Reactive` one through a `SignalFragment`, flattening a `Fragment` result into the pass so keyed children stay reconcilable), the JSX rendering components (`For`, `Show`, `Maybe`, `Value`, `Text`, `Int`, `Float`, `Bool`), attribute helpers (`attr`, `signalAttr`, `computedAttr`, `optionalAttr`, `optionalSignalAttr`, `optionalComputedAttr`, `Attr`), the `null`/`empty` placeholders, and `mount`/`mountById`. It also exposes the two helpers `@xote.component` emits: `child` (runtime coercion of a bare JSX child) and `probe` (hidden-read detection — see the PPX section below). Note that `child` is typed `'a => node`, so it **erases type checking in child position**: under the annotation a record or variant in `{…}` compiles and renders `"[object Object]"` with a development-only console warning (gated on the same `__XOTE_DEV__` / `NODE_ENV` flag as `probe`), where unannotated code would have failed to compile. That is the deliberate price of the zero-ceremony bare child. The renderer itself lives in the internal `RuntimeRender` module, the node types in `RuntimeNode`, the owner-based cleanup system in `RuntimeOwner`, and the template cloning that renders repeated shapes in `RuntimeTemplate`.
...
- **`Xote.MaybeSignal`**: Static-or-reactive value wrapper exposing the type `t<'a> = Reactive(Signal.t<'a>) | Static('a)` plus `static`, `reactive`, `computed` (derives a `Reactive` from a `unit => 'a`), `fold` (the single total branch over the two cases), `get` (tracked read), `peek` (untracked read), `isStatic`/`isReactive`, `map`, `toSignal`, and `ofUnknown`. Use it anywhere an API should accept either a plain value or a signal — JSX props are the most common case, not the only one. Notes on the trickier members:
  - `fold` takes `~static` and `~reactive` callbacks — the two are exactly the inverses of the `static` and `reactive` constructors, so `~reactive` receives the `Signal.t`, not its value. `get` and `peek` are built on it, and `View.render` is the same branch at the node level. It reads well in a pipe; `switch` on the constructors is equally fine and gives you exhaustiveness checking, so use whichever is clearer at the call site.
  - `map` and `computed` run their function once immediately. For a `Reactive` value the result is backed by a `Computed` that stays subscribed to the source until `Computed.dispose` — it is **not** marked library-owned, so no node releases it for you (that marking is reserved for computeds the library allocates to back a node). Hold on to long-lived mapped values rather than rebuilding them per update.
  - `toSignal` returns `Some(source signal)` for `Reactive` and `None` for `Static` — a plain value has no underlying signal, and the old behavior (lifting `Static` into a fresh, detached, writable signal) was a footgun. To lift a `Static` value into a signal explicitly, write `MaybeSignal.fold(value, ~static=v => Signal.make(v), ~reactive=s => s)` — `~static=Signal.make` does not typecheck, because `Signal.make` carries optional `~name`/`~equals` arguments.
...
**Internal modules** (prefixed `Runtime`, no compatibility guarantee): `RuntimeNode` (the `node`/`attrValue` types and `resolveAttr`/`peekAttr`), `RuntimeRender` (DOM rendering and keyed reconciliation), `RuntimeTemplate` (skeleton cloning for repeated node shapes), `RuntimeDom`, `RuntimeOwner`, `RuntimeHtml`, `RuntimeAttr`, `RuntimeValue`, `RuntimeJsxProp`, `RuntimeHydrationMarkers`.
...
**Owner System**: Reactive state is owned per *region* (the internal `RuntimeOwner` module): a keyed row, one pass of a `SignalFragment`, a hydrated row. Each region has one owner — a single array of disposers and library-owned computeds — and the reconciler disposes it when it removes the region, so nothing hangs off DOM nodes and disposal never walks the DOM. Three things register with the region that is rendering: the effects the renderer creates for reactive text and attributes, the computeds the library allocates to back a node (`View.child`, `tracked`, `each`, `Show`/`Maybe`/`Value`, `signalText`), and any `Effect.run` a component body sets up. A signal or computed the consumer built is never disposed on their behalf.
...
12. **Region-owned cleanup**: Reactive state (effects, computeds) belongs to the region that rendered it — a keyed row, a `SignalFragment` pass, a hydrated row — rather than to individual DOM nodes. A region has one owner, a flat array mixing effect disposers and owned computeds, and the reconciler disposes it when it drops the row or replaces the pass; a nested region (a list inside a row) registers its own release with the region rendering it, so disposal cascades without touching the DOM. The DOM carries no expando and no per-element scope: a static element costs nothing to own, and clearing a ten-thousand-row list runs ten thousand owner disposals instead of a hundred-thousand-node walk. `Effect.run`/`Effect.runWithDisposer` called while a component renders registers with the region rendering that component, so unmounting stops the effect; called outside a render (module level, an event handler) there is no region to belong to and the effect lives until its disposer runs.

13. **Keyed list reconciliation**: `eachWithKey` uses comment-based anchors and a single pass over the new order: a tracked key is kept if its item identity is unchanged and rebuilt in place if not, a new key is built, and stale rows are whatever the row map still holds with an older pass stamp — found without materialising the key set, and not looked for at all when nothing was dropped. Rows are tracked in a `Map` and remember their position after each pass, which is also their relative DOM order, so placement never reads the live children. When the survivors kept their relative order (an append, a removal, a creation from empty) nothing in the document moves and only the fresh rows are inserted, gathered per run into a fragment; otherwise the ordering pass is **minimal**: it leaves in place a longest increasing subsequence of the survivors' previous positions and moves only the rest, so swapping two rows of a thousand costs two `insertBefore` calls rather than one per row. `RuntimeRender.reconcileKeyed` is shared by this pass and the keyed `SignalFragment` path, and the whole list pass is shared by the render path and by hydration, so a hydrated list cannot drift from a rendered one.
14. **Template cloning**: a region renders its repeated shapes by cloning. The first tree of a given root a region sees is rendered node by node; the second is rendered while a static *skeleton* of it is built alongside (elements, static attributes and text, empty placeholders for reactive leaves and dynamic children) together with a *shape* recording what stood at each position; from then on a tree with that root is `cloneNode(true)` of the skeleton, walked against the shape to attach handlers and effects and to write the text and attribute values that differ. The match is soft — a subtree that does not fit its shape is rendered normally and swapped in — so a heterogeneous list still clones what its rows share, and a static value that turns out to vary between rows flips its position to compare-and-set. Text that is an element's only child goes through `textContent`, so no text node is created from script. Up to three skeletons are kept per region (`RuntimeTemplate.maxTemplates`); a region that renders once never builds one. Measured by `benchmarks/dom-ops.mjs`, a thousand rows cost ~2,000 DOM calls instead of ~20,000.

14. **SSR hydration markers**: Comment nodes mark reactive boundaries in server-rendered HTML. The hydration walker uses these to attach reactivity without re-rendering the DOM. A keyed list's own `<!--kl-->` / `<!--/kl-->` markers become its anchors — the end marker is the tail the reconciler inserts before — which is what lets hydration **adopt** the server's rows rather than rebuild them: each row is hydrated through the ordinary path so its handlers and reactive attributes attach to the node the server sent, and it is then reconciled in place. A `SignalFragment` is not adopted this way: its first effect pass clears the container and re-renders.
...
MaybeSignal.fold(label, ~static=v => v, ~reactive=Signal.peek) // one total branch
MaybeSignal.toSignal(label)     // option<Signal.t<string>>; None for a Static input
...
4. **Manual JSX key plumbing**: a JSX `~key` (`<li key={id}>`) wraps the element in a `Keyed` node, but that node only takes part in reconciliation when *every* child of a reactive fragment pass is keyed (e.g. the items of `View.each`) — and even there, element identity survives only while the item's props stay shallow-equal between passes, which any child node or freshly-allocated handler closure defeats, so keyed items with children are rebuilt anyway. In a static children array the key has no effect at all. Use `View.eachWithKey` (or `<View.For by=...>`), which keys on *your item* and reconciles by item identity, rather than relying on JSX-level keys.
...
6. **Templates are learned at runtime, not compiled**: a region clones a skeleton only from its second tree of a given root onwards (see architecture note 14), so the first row of every list and every one-off render still pay node-by-node construction, and a region whose rows share no structure at all — more than three distinct roots — gets no cloning past the third. The shape check walks the tree in JavaScript on every render, which a compiler would do once.
7. **A keyed row must have a single element root**: `eachWithKey`/`<View.For by=…>` remember one node per key, so a row that renders to a `Fragment` — including a component whose root is `<>…</>` — is remembered as the fragment, which appending empties. Such rows render correctly and are removed correctly, but **reordering them does nothing**, because moving an empty fragment moves none of the nodes that came out of it. Give a keyed row a single wrapping element when its order can change.
...
| `src/RuntimeRender.res` | Internal renderer and keyed reconciler, shared by `View.mount` and `Hydration` |
| `src/RuntimeTemplate.res` | Internal template cloning: skeleton and shape per repeated node shape, soft-matched fill |
| `src/RuntimeOwner.res` | Internal region ownership of effects and owned computeds |
...
- **Owner disposal**: Reactive state is released per region by the reconciler (`RuntimeOwner.disposeOwner` on the row or pass owner), not per DOM node — anything the renderer creates for a row must be created while that row's owner is current (`RuntimeOwner.runWithOwner`), or nothing will ever dispose it
