...
- **`Xote.MaybeSignal`**: Static-or-reactive value wrapper exposing the type `t<'a> = Reactive(Signal.t<'a>) | Static('a)` plus `static`, `reactive`, `computed` (derives a `Reactive` from a `unit => 'a`), `fold` (the single total branch over the two cases), `get` (tracked read), `peek` (untracked read), `isStatic`/`isReactive`, `map`, `toSignal`, and `ofUnknown`. Use it anywhere an API should accept either a plain value or a signal — JSX props are the most common case, not the only one. Notes on the trickier members:
  - `fold` takes `~static` and `~reactive` callbacks and is the one place the variant is inspected; `get` and `peek` are built on it. Prefer `fold` (or `View.render`) over matching the constructors directly.
  - `map` runs its function once immediately in both cases. For a `Reactive` value the result is backed by a `Computed`; inside a component that `Computed` is registered with the active owner and disposed on unmount, while outside one you must call `Computed.dispose` on it yourself.
  - `toSignal` returns `Some(source signal)` for `Reactive` and `None` for `Static` — a plain value has no underlying signal, and the old behavior (lifting `Static` into a fresh, detached, writable signal) was a footgun. Use `fold` to lift a `Static` value into a signal explicitly when you need a uniform `Signal.t`.
...
4. **Computed disposal**: `Computed.make` returns a `Signal.t<'a>` directly, already subscribed to whatever it read during its initial computation. There is no automatic disposal — call `Computed.dispose(signal)` to unlink a computed you no longer need. Xote's owner system tracks `Effect` disposers and, for computeds created via `MaybeSignal.computed`/`MaybeSignal.map`, the resulting `Computed` as well; a raw `Computed.make` during render is still your responsibility to dispose.
