...
This is the load-bearing finding of the whole exercise. Everything in `xote-native/`
...
**This is the layout on disk, not a proposal** — the extraction is done.

```
xote-native/                          xote/  (unchanged, web — the repo root)
...
├── example/                 the counter, the panel, the tracker
├── hosts/ios/               ─┐  still here, and each wants to be its own
├── hosts/android/           ─┘  artifact — a Swift package and a Gradle
└── test/                        module — which is the next move
```

The two live in one repository, so `xote-native` needs a `node_modules/xote` to
resolve — `scripts/link-workspace.mjs` makes that symlink and every script runs
it first. A downstream consumer gets the same arrangement from `npm install` and
needs none of it.
...
**This is no longer a proposal.** `xote-native/test/package_test.mjs` stages `xote`
and a synthetic `xote-native` into a temporary `node_modules`, compiles the
fixture app in `xote-native/test/__fixtures__/package/` against both, and asserts
...
| | |
|---|---|
| **Moves** to `xote-native` | `xote-native/src/*.res`, `xote-native/src/host/`, `xote-native/conformance/`, `xote-native/test/`, `xote-native/example/`, and the five native npm scripts |
...
| **Moves out separately** | `xote-native/hosts/ios/` and `xote-native/hosts/android/` each want to be their own artifact — a Swift package and a Gradle module, not directories in a JavaScript one. They are `xote-native`'s reference hosts, not part of it. `native/` is already laid out that way |
...
2. ~~**Extend the boundary test to two packages**, still in this repository,
   with `native/` still where it is.~~ **Done** — `xote-native/test/package_test.mjs`,
   and it found the two problems above.
3. ~~**Lift `native/` into `xote-native`.**~~ **Done.** It cost exactly what the
   build said it would: a `rescript.json` with `-open Xote`, a `package.json`
   with an `exports` map, and `host/` living inside `src/`. **No source file
   changed** — only import paths in the tests and the bundler, which are not
   part of the package's own code.
4. **Split the hosts out** — `hosts/ios/` into a Swift package, `hosts/android/`
   ...
   The directory is already arranged so this is a move rather than an
   untangling.
...
Everything else is genuinely inert, and now structurally so: `xote-native` is a
separate ReScript and npm package with its own `rescript.json` and its own
`package.json`. It is not a source directory of `xote`, `npm run res:build` at
the root does not compile it, and no arrangement of `files` could put it in the
`xote` tarball by accident.
...
3. **Does the protocol survive a second platform?** Mostly answered. A Kotlin
   host now exists (§7.4), and writing it changed nothing in `xote-native/src/host/`, in
