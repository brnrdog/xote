...
### Native (prototype)
- `npm run native:test` - End-to-end test of the native rendering prototype: the type surface, the protocol handshake, the package split, the layout oracle, the host conformance suite, resilience, the windowed list, view flattening and recycling, and the tracker measurements
- `npm run native:package` - Stage both packages into a temporary `node_modules` the way `npm install` would and compile a downstream app against them
...
- `npm run native:bundle` / `npm run native:bundle:test` - Build the one bundle both platforms share, and run it in a bare realm (no DOM, no `console`, no timers) that stands in for an embedded JavaScript engine. `packages/xote-native/hosts/` carries the platform hosts, neither of which this repository can compile — see their READMEs.
- `xote-native` is a **separate package** in this repository — `packages/xote-native/`, with its own `rescript.json`, `package.json` and tests. It depends on `xote` and resolves it by package name through the workspace link, exactly as a downstream consumer would. `npm run res:build` here does not build it. See [`../../AGENTS.md`](../../AGENTS.md) for the repository layout and [`../xote-native/README.md`](../xote-native/README.md) for the architecture.
...
- **Native findings report**: `../xote-native/REPORT.md` - how that package is built on Xote's primitives and packaged separately from the web library, what `xote` had to change, and an assessment of the demo
