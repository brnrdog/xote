...
> transliteration of `native/host/layout.mjs`, which is checked frame-for-frame
...
cd native/hosts/android && ./gradlew installDebug
```
...
out of `native/bundle/dist/` — the same file the iOS host uses — rather than
...
`XoteConformanceTest` replays `native/conformance/suite.json` against real
`View`s and compares the node tree, every frame, the text, and the view tree.
Gradle points the test assets at `native/conformance/` directly, so the file is
