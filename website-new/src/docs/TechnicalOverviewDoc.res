...
let content = () => {
  <div>
    ...
    <div class="info-box">
      <p>
        ...
        {Component.text(
          "Xote v3.0+ uses rescript-signals for all reactive primitives (Signal, Computed, Effect). This overview focuses on Xote-specific features: Components, Router, and JSX support.",
        )}
      </p>
    </div>
    <h2> {Component.text("Architecture Overview")} </h2>
    <h3> {Component.text("Module Structure")} </h3>
    <p> {Component.text("Xote is organized into focused modules:")} </p>
    <ul>
      <li>
    ...
    <ul>
      <li>
        ...
        {Component.text(
          " When an observer (effect or computed) runs, any Signal.get calls register the signal as a dependency",
        )}
      </li>
      <li>
        <strong> {Component.text("Scheduling:")} </strong>
        {Component.text(
          " When Signal.set is called, all dependent observers are scheduled and run synchronously",
        )}
      </li>
      <li>
        <strong> {Component.text("Push-based Computeds:")} </strong>
        {Component.text(
          " Computeds eagerly recompute when dependencies change and push results to their backing signal",
        )}
      </li>
      <li>
        <strong> {Component.text("Structural Equality:")} </strong>
        {Component.text(
          " Signals use structural equality (==) to check if values have changed, preventing unnecessary updates",
        )}
      </li>
    </ul>
    <h2> {Component.text("Component System")} </h2>
    <h3> {Component.text("Virtual Node Types")} </h3>
    <p> {Component.text("Xote uses several node types to represent UI elements:")} </p>
    <ul>
      <li>
    ...
    <ul>
      <li>
        <strong> {Component.text("SignalText:")} </strong>
        {Component.text(
          " Creates a DOM text node and sets up an effect that updates textContent when the signal changes",
        )}
      </li>
      <li>
        <strong> {Component.text("SignalFragment:")} </strong>
        {Component.text(
          " Uses a container element with display: contents and replaces all children when the signal changes (no diffing)",
        )}
      </li>
      <li>
        <strong> {Component.text("Lists:")} </strong>
        {Component.text(
          " Implemented as a computed signal + SignalFragment, so the entire list rerenders on any array change",
        )}
      </li>
      <li>
        <strong> {Component.text("Reactive attributes:")} </strong>
        {Component.text(
          " Set up effects that update the DOM attribute when the signal/computed value changes",
        )}
      </li>
    </ul>
  ...
  "jsx": {
    ...
    <p> <strong> {Component.text("Features:")} </strong> </p>
    <ul>
      <li> {Component.text("Lowercase tags for HTML elements")} </li>
      <li> {Component.text("Props support for common attributes and events")} </li>
      <li> {Component.text("Children passed via JSX syntax")} </li>
      <li> {Component.text("Component functions called with props objects")} </li>
    </ul>
    <h2> {Component.text("Router Architecture")} </h2>
    <h3> {Component.text("Route Matching")} </h3>
    <p> {Component.text("Pattern-based string matching with :param syntax:")} </p>
    <ul>
      <li>
      ...
      <li> {Component.text("Parameters returned as Dict.t<string>")} </li>
    </ul>
    ...
    <ul>
      <li>
        ...
        {Component.text(
          " Signals push notifications to observers; computeds eagerly push into their backing signal",
        )}
      </li>
      <li>
    ...
    <ul>
      <li>
        ...
        <ul>
          <li> {Component.text("Automatic dependency tracking on read")} </li>
          <li> {Component.text("Observer-based recomputation and re-tracking")} </li>
          <li> {Component.text("Structural equality checks")} </li>
        </ul>
      </li>
      <li>
        ...
        <ul>
          ...
          <li> {Component.text("Synchronous scheduling rather than microtask-based")} </li>
          <li> {Component.text("Effects can return cleanup callbacks (Some/None pattern)")} </li>
        </ul>
      </li>
    </ul>
