...
let isActive = Signal.make(true)`)}
    ...
    <p>
      ...
      <code> {Component.text("Signal.get()")} </code>
      {Component.text(
        " to read a signal's value. When called inside a tracking context (like an effect or computed value), it automatically registers the signal as a dependency:",
      )}
    </p>
    <pre>
      <code>
...
Effect.run(() => {
  ...
  Console.log2("Peeked:", peeked)
    ...
    <p> {Component.text("Replace a signal's value entirely:")} </p>
    <pre>
      <code>
...
Signal.set(count, 10) // count is now 10`)}
    ...
    <h3>
      <code> {Component.text("Signal.update()")} </code>
    </h3>
    <p> {Component.text("Update a signal based on its current value:")} </p>
    <pre>
      <code>
        {Component.text(`let count = Signal.make(0)
...
Signal.update(count, n => n * 2) // count is now 2`)}
    ...
    <p>
      ...
      {Component.text(
        ") to check if a value has changed. If the new value equals the old value, dependents are not notified:",
      )}
    </p>
    <pre>
      <code>
...
let fullName = Computed.make(() =>
  Signal.get(firstName) ++ " " ++ Signal.get(lastName)
    ...
    <p> {Component.text("Here's a complete example showing signals in action:")} </p>
    <pre>
      <code>
        {Component.text(`open Xote
    ...
    <ul>
      <li>
        ...
        {Router.link(
          ...
          ~children=[Component.text("Computed Values")],
          (),
        )}
