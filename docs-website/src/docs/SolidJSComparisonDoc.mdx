...
Median of 15 iterations, Chromium 141 on a 4-core Xeon. Lower is better. These are ratios from one machine, not absolute performance claims.

| Operation | Xote | SolidJS |
| --- | --- | --- |
| Create 1,000 rows | 85.0 ms | 49.8 ms |
| Replace 1,000 rows | 122.7 ms | 68.3 ms |
| Update every 10th row | 6.7 ms | 7.2 ms |
| Select a row | 1.1 ms | 0.9 ms |
| Swap two rows | 70.1 ms | 5.8 ms |
| Remove a row | 5.9 ms | 5.4 ms |
| Create 10,000 rows | 924 ms | 646 ms |
| Clear 10,000 rows | 189 ms | 67 ms |
| Time to first render | 23.9 ms | 24.8 ms |
| App bundle, gzipped | 7.8 KB | 4.7 KB |

Because both use the same reactive model, targeted updates land in the same range: updating scattered rows and selecting a row are within noise of each other, and both write single text nodes rather than diffing.
...
Swapping two rows is the one result that is not a constant-factor difference. Xote's keyed reconciler walks the new order against the live DOM and inserts every node whose position does not match, so a single early mismatch cascades: swapping two rows costs 997 `insertBefore` calls where SolidJS costs 2. Lists that reorder frequently will feel this until the reconciler computes a minimal move set.
