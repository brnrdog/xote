...
| --- | --- | --- |
| Create 1,000 rows | 76.3 ms | 43.7 ms |
| Replace 1,000 rows | 87.4 ms | 50.0 ms |
| Update every 10th row | 4.2 ms | 4.4 ms |
| Select a row | 0.7 ms | 0.5 ms |
| Swap two rows | 43.1 ms | 4.0 ms |
| Remove a row | 4.0 ms | 3.6 ms |
| Create 10,000 rows | 844 ms | 520 ms |
| Clear 10,000 rows | 176 ms | 61 ms |
| Time to first render | 18.8 ms | 17.1 ms |
| App bundle, gzipped | 7.9 KB | 4.7 KB |
...
Swapping two rows is the one result that is not a constant-factor difference. Xote's keyed reconciler walks the new order against the live DOM and inserts every node whose position does not match, so a single early mismatch cascades: swapping two rows costs 997 `insertBefore` calls where SolidJS costs 2, and about 10x the time. Lists that reorder frequently will feel this until the reconciler computes a minimal move set.
