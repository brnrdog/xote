...
- SolidJS reads via function calls like `count()`, Xote reads via [`Signal.get`](/docs/api/signal#signal-get)
...
The main ergonomic difference is that SolidJS can embed reactive expressions directly inside JSX, while Xote makes the reactive boundary explicit with JSX primitives such as [`View.Int`](/docs/api/view#view-int-component), [`View.Text`](/docs/api/view#view-text-component), and [`View.For`](/docs/api/view#view-for).
...
SolidJS uses control-flow helpers like `<For>` and `<Index>`. Xote exposes the same idea through [`View.For`](/docs/api/view#view-for), with an optional `by` prop for keyed reconciliation.
...
The repository ships a [keyed-list benchmark](https://github.com/brnrdog/xote/tree/main/benchmarks) that runs the same table application in Xote, React, Vue, and SolidJS. The two implementations here are structurally the same program: one signal holds the list, each row owns a signal for its label, and updates are batched.
...
| --- | --- | --- |
| Create 1,000 rows | 53.5 ms | 49.3 ms |
| Replace 1,000 rows | 76.1 ms | 75.1 ms |
| Update every 10th row | 6.5 ms | 5.8 ms |
| Select a row | 0.6 ms | 0.9 ms |
| Swap two rows | 5.6 ms | 5.2 ms |
| Remove a row | 5.0 ms | 4.8 ms |
| Create 10,000 rows | 735 ms | 664 ms |
| Clear 10,000 rows | 76.1 ms | 66.1 ms |
| Time to first render | 19.2 ms | 19.5 ms |
| App bundle, gzipped | 10.4 KB | 5.5 KB |
...
Creating rows is where the two approaches to templates meet. SolidJS's JSX compiler emits a template that is cloned per row, so building 1,000 rows costs about 2,000 DOM calls. Xote learns the same thing at runtime: a keyed list records the skeleton of its rows from the second row it renders and clones it for the rest, so it makes about the same 2,000 DOM calls and creation lands within a fifth of SolidJS at 1,000 rows. The difference shows on the larger sizes — at 10,000 rows the view tree Xote builds and discards per row costs it garbage-collection time a compiled template never allocates — and in the heap, where each Xote row carries its view tree's reactive bookkeeping.

Reordering is a tie. Both keyed reconcilers compute a minimal move set — the rows already in the right relative order are left alone — so a two-row swap costs two `insertBefore` calls in either library.
...
1. signal reads move from function calls to [`Signal.get`](/docs/api/signal#signal-get)
2. reactive JSX expressions often become [`View.Text`](/docs/api/view#view-text-component), [`View.Int`](/docs/api/view#view-int-component), [`View.For`](/docs/api/view#view-for), or another explicit [`View`](/docs/api/view#view-module) primitive
