...
| --- | --- | --- |
| Create 1,000 rows | 39.5 ms | 41.9 ms |
| Replace 1,000 rows | 44.4 ms | 48.2 ms |
| Update every 10th row | 5.1 ms | 8.4 ms |
| Select a row | 0.6 ms | 4.5 ms |
| Swap two rows | 4.5 ms | 47.2 ms |
| Remove a row | 3.7 ms | 7.7 ms |
| Create 10,000 rows | 534 ms | 807 ms |
| Clear 10,000 rows | 64.4 ms | 86.1 ms |
| Time to first render | 18.6 ms | 34.8 ms |
| App bundle, gzipped | 10.8 KB | 59.9 KB |

The results follow the architecture. Scattered updates and selection are where fine-grained reactivity pays: Xote writes only the text nodes that changed, while React re-renders the list and diffs it, so Xote runs about 2x faster on the update and several times faster on selection. Building large lists used to go the other way; Xote now clones a skeleton it learns from the first rows of a list instead of constructing every node, and creates rows faster than React does.
