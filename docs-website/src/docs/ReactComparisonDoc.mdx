...
In Xote, reactive state belongs to the row that created it rather than to individual DOM nodes, so removing rows disposes one owner per row instead of walking every node of the removed subtree, and Xote clears the list faster than React does.
