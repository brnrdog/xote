...
| --- | --- | --- |
| Create 1,000 rows | 85.5 ms | 63.8 ms |
| Replace 1,000 rows | 120.6 ms | 77.5 ms |
| Update every 10th row | 7.8 ms | 13.4 ms |
| Select a row | 1.1 ms | 6.6 ms |
| Swap two rows | 57.0 ms | 63.7 ms |
| Remove a row | 9.2 ms | 12.8 ms |
| Create 10,000 rows | 961 ms | 1017 ms |
| Clear 10,000 rows | 187 ms | 121 ms |
| Time to first render | 23.2 ms | 48.0 ms |
| App bundle, gzipped | 7.8 KB | 59.9 KB |

The results follow the architecture. Scattered updates and selection are where fine-grained reactivity pays: Xote writes only the text nodes that changed, while React re-renders the list and diffs it, so Xote runs about 1.7x faster on the update and 6x faster on selection. Building large lists goes the other way, because React's element creation path is more optimized than Xote's node-by-node construction.
