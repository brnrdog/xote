...
| --- | --- | --- |
| Create 1,000 rows | 85.0 ms | 58.4 ms |
| Replace 1,000 rows | 122.7 ms | 73.9 ms |
| Update every 10th row | 6.7 ms | 14.8 ms |
| Select a row | 1.1 ms | 5.4 ms |
| Swap two rows | 70.1 ms | 65.8 ms |
| Remove a row | 5.9 ms | 11.0 ms |
| Create 10,000 rows | 924 ms | 961 ms |
| Clear 10,000 rows | 189 ms | 113 ms |
| Time to first render | 23.9 ms | 43.5 ms |
| App bundle, gzipped | 7.8 KB | 59.9 KB |

The results follow the architecture. Scattered updates and selection are where fine-grained reactivity pays: Xote writes only the text nodes that changed, while React re-renders the list and diffs it, so Xote runs about 2.2x faster on the update and nearly 5x faster on selection. Building large lists goes the other way, because React's element creation path is more optimized than Xote's node-by-node construction.

Clearing a large list is Xote's weakest result here. Removal walks the owner tree to dispose per-node effects, which a virtual DOM renderer does not have to do.

Swapping two rows is slow in both, for the same reason: neither reconciler computes a minimal set of moves, so both issue close to one `insertBefore` per row. Vue and SolidJS do it in two.
