...
> **Important:** [`Computed.make`](/docs/api/computed#computed-make) returns a [`Signal.t<'a>`](/docs/api/signal#signal-type). You read it with [`Signal.get`](/docs/api/signal#signal-get) or [`Signal.peek`](/docs/api/signal#signal-peek), just like any other signal.
...
If you need to suppress downstream updates for equivalent derived values, [`Computed.make`](/docs/api/computed#computed-make) also accepts `~equals`.

### Reading Computed Values

Read a computed exactly like a signal. Use [`Signal.get`](/docs/api/signal#signal-get) when the current code should subscribe, or [`Signal.peek`](/docs/api/signal#signal-peek) for a one-off read.
...
If you create a long-lived computed outside normal component ownership and want to tear it down explicitly, call [`Computed.dispose`](/docs/api/computed#computed-dispose).
...
- Read computeds with [`Signal.get`](/docs/api/signal#signal-get) or [`Signal.peek`](/docs/api/signal#signal-peek), because they are signals at the type level.
