...
Use `Router.location()` to access the shared location signal. Read it with [`Signal.get`](/docs/api/signal#signal-get) wherever you need the current snapshot.
...
let nav = () => {
  <nav>
    <Router.Link to="/" class="nav-link">
      <View.Text> "Home" </View.Text>
    </Router.Link>
    <Router.Link to="/docs" class="nav-link">
      <View.Text> "Docs" </View.Text>
    </Router.Link>
  </nav>
}
```
...
let nav = () => {
  <nav>
    ...
    <Router.Link to="/users"> <View.Text> "Users" </View.Text> </Router.Link>
  </nav>
}
