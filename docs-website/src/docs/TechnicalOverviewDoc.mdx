...
> **Scope:** This is architecture-level guidance, not a substitute for the module API reference pages.
...
- [`Signal`](/docs/api/signal#signal-module), [`Computed`](/docs/api/computed#computed-module), [`Effect`](/docs/api/effect#effect-module) - state, derived state, and side effects
- [`View`](/docs/api/view#view-module) and `Html` - UI node constructors, mounting, and HTML helpers
...
- **Tracked reads:** [`Signal.get`](/docs/api/signal#signal-get) subscribes the active observer
- **Synchronous scheduling:** updates flush immediately unless wrapped in [`Signal.batch`](/docs/api/signal#signal-batch)
...
The public view variants cover the main cases: text, elements, fragments, value rendering, conditionals, lazy components, and lists. JSX apps usually consume the [`View`](/docs/api/view#view-module) primitives first; lower-level constructors remain available when building custom abstractions.
...
- [Use the Signal API page](/docs/api/signal) as the quick reference while reading the architecture back into the code.
