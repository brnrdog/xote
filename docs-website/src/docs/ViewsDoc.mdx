...
JSX expressions are just nodes. For reactive text, use `View.signalText` or the value primitives such as `View.Text` and `View.Int`. For arrays and lists, use `View.For`; pass `by` when item identity matters.
...
<ul>
  <View.For
    ...
    render={item => <li> {View.text(item)} </li>}
  />
</ul>
```
...
<ul>
  <View.For
    ...
    by={todo => todo.id}
    render={todo => <li> {View.text(todo.text)} </li>}
  />
</ul>
```
...
<View.Maybe
  ...
  fallback={<p> {View.text("No todo selected")} </p>}
  render={todo => <p> {View.text(todo.text)} </p>}
/>
