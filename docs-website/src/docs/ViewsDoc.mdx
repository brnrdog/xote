...
module Greeting = {
  ...
  let make = (~name: string, ~emphasis=false) => {
    <div class={emphasis ? "greeting strong" : "greeting"}>
      <h1> <View.Text value={Prop.static(`Hello, ${name}`)} /> </h1>
    </div>
  }
}
...
module Greeting = {
  ...
  let make = (~name: string) => {
    <section class="greeting-card">
      <h2> <View.Text value={Prop.static(`Hello, ${name}`)} /> </h2>
      <p> <View.Text value={Prop.static("This component is ordinary ReScript plus JSX.")} /> </p>
    </section>
  }
}
```
...
JSX expressions are just nodes. For direct value output, use primitives such as `View.Text`, `View.Int`, `View.Float`, and `View.Bool`. For arrays and lists, use `View.For`; pass `by` when item identity matters.
...
<button
  ...
  onClick={toggle}>
  <View.Text value={Prop.static("Toggle")} />
</button>
```
...
<ul>
  <View.For
    ...
    render={item => <li> <View.Text value={Prop.static(item)} /> </li>}
  />
</ul>
```
...
<ul>
  <View.For
    ...
    by={todo => todo.id}
    render={todo => <li> <View.Text value={Prop.static(todo.text)} /> </li>}
  />
</ul>
```
...
```rescript
<View.Show when_={Prop.signal(isReady)} fallback={<p> <View.Text value={Prop.static("Loading")} /> </p>}>
  <p> <View.Text value={Prop.static("Ready")} /> </p>
</View.Show>

<View.Maybe
  value={Prop.signal(selectedTodo)}
  fallback={<p> <View.Text value={Prop.static("No todo selected")} /> </p>}
  render={todo => <p> <View.Text value={Prop.static(todo.text)} /> </p>}
/>
...
<View.Value
  ...
  render={count =>
    <p>
      <View.Text value={Prop.static("Count: ")} />
      <View.Int value={Prop.static(count)} />
    </p>
  }
/>
...
```rescript
let app = () => {
  <div> <View.Text value={Prop.static("Hello, Xote")} /> </div>
}
...
module TodoComposer = {
  ...
  let make = () => {
    <div>
      ...
      <button onClick={addTodo}> <View.Text value={Prop.static("Add")} /> </button>
    </div>
  }
}
...
module TodoRow = {
  ...
  let make = (~todo: todo) => {
    <li>
      <span> <View.Text value={Prop.static(todo.title)} /> </span>
