...
### The `@tracked` annotation (experimental, opt-in)

There is an experimental compile-time alternative that removes the wholesale-replacement tradeoff. An optional native PPX lets you annotate a JSX block with `@tracked` and read signals **inline** — no `() => …` thunks, no `View.signalText`:
