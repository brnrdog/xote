...
Prefer no PPX at all? Omit `ppx-flags` and use `@jsx.component` instead of `@xote.component` — everything below still works, you just write reactive attributes/text with explicit `() => …` thunks.
