...
### The `@xote.component` annotation (experimental, opt-in)

There is an experimental compile-time alternative that removes the wholesale-replacement tradeoff. An optional native PPX lets you annotate a component with `@xote.component` and read signals **inline** — no `() => …` thunks, no `View.signalText`:
...
`@xote.component` replaces `@jsx.component`: it derives props from the labeled args **and** fine-grains the returned JSX. Rather than wrapping the whole block in one computed, the PPX **decomposes** it: each attribute or text that reads a signal becomes its own reactive leaf (a `computedAttr` / reactive text node), and `View.tracked` is emitted only around a child region whose node *structure* varies (an `if`/`switch`), tracking just the condition. The enclosing elements are built once and keep their DOM identity — so it stays fine-grained even for large components. This is what the Counter demo above is written with.

It is **opt-in** and not part of the published package — it requires a small native binary you build yourself (the only dependency is `ocamlopt`):
...
then add it to *your* project's `rescript.json`:

```json
{ "ppx-flags": ["xote/ppx/ppx"] }
```

Consumers who don't opt in are unaffected — xote's own published config carries no PPX. See [`ppx/README.md`](https://github.com/brnrdog/xote/blob/main/ppx/README.md) for the decomposition rules, signal-detection details, and limitations.
